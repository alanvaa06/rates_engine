...
- AC-3.2: Given la curva **resultante de un bootstrap**, When se inspecciona, Then todo DF es positivo y no creciente; si los inputs implican lo contrario se lanza `CurveArbitrageError` nombrando el tramo. **Corregido en implementación:** la monotonía se verifica al final del bootstrap, no en el constructor de `DiscountCurve`. Razón: todo DV01 de este paquete es una diferencia simétrica, y bajar 1 bp un forward de 1 bp cruza cero legítimamente — prohibirlo en el constructor dejaría el DV01 indefinido en entornos de tasa baja, y las tasas negativas existen. Un DF no positivo sí se rehusa siempre. La curva expone `rising_segments` y `require_monotone()`.
...
- AC-6.5: Given `dv01(instrument, curve_set, bump_bp=1)`, When se calcula, Then es un shift **paralelo** de la curva cero por **diferencia central** con re-precio completo, y la evidencia declara `bump_bp`, `bump_basis`, `bump_shape` y unidad (USD/bp). La diferencia central no es cosmética: cancela el término de segundo orden, que es lo que permite que payer y receiver coincidan a precisión de máquina (AC-6.2) y que el perfil key rate sume de vuelta (AC-10.2). Key rate, duración y convexidad viven en **US-10**.
...
- AC-10.2: Given esos bumps, When se suman los KR DV01, Then iguala el DV01 paralelo de AC-6.5 a **1e-6 relativo**. (Este es el test del viejo AC-6.5, ahora con la condición que lo hace cierto.) **Precisión añadida en implementación:** la tolerancia es relativa, no absoluta. El residuo es el término de tercer orden de una diferencia simétrica: diminuto pero no cero, y 1e-6 absoluto sobre un DV01 de veinte mil dólares sería exigir exactitud de bits con otro nombre. Medido: 3e-10 relativo.
...
- AC-10.7 **(reescrito en implementación — el original era falso)**: Given las dos vistas sobre el mismo swap y la misma curva, When se comparan, Then **cada una suma su propio paralelo a 1e-6 relativo** — la de nodo suma `dv01` (shift paralelo de la curva cero), la de instrumento suma `quote_parallel_dv01` (shift simultáneo de todas las cotizaciones) — y el payload reporta `node_vs_instrument_difference` **sin declarar que una es la correcta**.

  El AC original exigía que ambas sumaran *el mismo* número. Eso es matemáticamente falso: son derivadas respecto de variables distintas. Un punto base sobre un forward simple ACT/360 no es un punto base sobre una tasa cero compuesta continuamente, y el stub realizado no es una cotización que se mueva; medido, la razón es 1.0037, entre 1 y 365/360. Lo que sí es una identidad exacta — y lo que el test verifica ahora — es que una suma de derivadas parciales iguala la derivada en la diagonal, calculada por una ruta independiente. Esa versión sí puede fallar cuando hay un error de signo; la original solo podía fallar siempre.
...
- AC-10.10 **(reescrito en implementación — el original mezclaba bases)**: dos afirmaciones, cada una en una sola base.
  1. Given el swap del `shock_table`, When se predice su P&L con `-dv01·Δbp + ½·money_convexity·Δy²`, Then coincide con el re-precio completo dentro de **1% a ±10 bp y 10% a ±100 bp**. Medido: 1e-6 y 1e-4. Esto valida DV01 y convexidad conjuntamente contra el re-precio, todo en la base de curva cero.
  2. Given el **neto** de la posición cubierta, When se recorre la rejilla de shocks, Then es de segundo orden: `net(±10 bp)` coincide en signo y magnitud, `net(100)/net(10) ≈ 100 ±5%`, y la columna `implied_position_money_convexity` (`2·net/Δy²`) es constante a menos de 5% en toda la rejilla. Medido: 1.5%.

  El original comparía `money_convexity` del swap (base curva cero) contra el *neto* de la posición cubierta, que incluye la tira. Son bases distintas y la comparación fallaba al 8-9% — no por un error de cálculo sino porque la pregunta estaba mal planteada.

  **Defecto real encontrado por esta reconciliación:** el P&L de la tira se estaba calculando con un punto base *nominal* por contrato. Un contrato de futuros paga linealmente en **su propio forward**, y un shift paralelo de la curva cero no mueve ese forward exactamente un punto base. El error dejaba un residuo de primer orden que sepultaba la convexidad (74% de discrepancia a 10 bp) y hacía que el hedge ratio se reportara como -1.0037 estando bien. Corregido: el P&L de la tira usa el movimiento real del forward de cada contrato, con lo que el hedge queda neutral a primer orden bajo **cualquier** shock y el hedge ratio medido es -0.99999999.
...
v0.1.0 construido. `pytest -q`: **508 pasan, 5 saltan**. `ruff check src tests scripts` limpio. `mypy src/rates_engine` limpio en 32 archivos, con allowlist vacía (ceiling 0).

Los 5 tests que saltan son exactamente los que comparan contra números publicados por un tercero (AC-5.1, AC-5.2, AC-6.6, AC-8.1, AC-8.2). **Resultado de la tarea 0.4 del plan:** el entorno de construcción tenía egress limitado a registries de paquetes y GitHub por política de la organización; `fred.stlouisfed.org` devolvió 403 en el proxy y CME era inalcanzable, de modo que ni el whitepaper, ni los settlements finales, ni los fixings pudieron traerse. No se sustituyó nada: un fixture generado convertiría un test que prueba algo en un test que prueba que el generador coincide consigo mismo. Las tolerancias no se movieron y los tests se activan solos en cuanto los archivos aparezcan en `tests/fixtures/cme_published/` — sin cambiar código. La cláusula de A1 prevé exactamente esto.

Lo que sí corre en su lugar: el settlement SR1/SR3 se verifica contra una reimplementación independiente escrita desde la definición (atrapa una fórmula equivocada, no una convención equivocada — ese hueco es justo lo que el test saltado llena), y la maquinaria de cobertura se verifica en forma y magnitud (700-900 contratos para un 2y de 100M, DV01 entre 18k y 24k).

Pendiente de `[manual-check]`: la regla de observancia SIFMA sábado→viernes (AC-1.2). Implementada, no confirmada contra la lista publicada; la procedencia del fixture lo dice.
